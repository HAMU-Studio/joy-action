...
public class Joycon
{
    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                DebugPrint(string.Format("Dequeue. Queue length: {0:d}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3:t}. Lag between packets (expect 15ms): {4:g}", reports.Count, report_buf[0], report_buf[1], System.DateTime.Now.Subtract(rep.GetTime()), rep.GetTime().Subtract(ts_prev)), DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
    ...
    //â¡ë¨ìxÇÃåüímÇÕÇ±ÇÃâ∫ÇÃìÒå¬Ç¡Ç€Ç¢
    ...
    /// <summary>
    /// joyconÇÃêUìÆã@î\ÇæÇ∆évÇÌÇÍÇÈ
    /// </summary>
    /// <param name="low_freq">é¸îgêî(îg)ÇÃâ∫å¿ÅH</param>
    /// <param name="high_freq">é¸îgêî(îg)ÇÃè„å¿ÅH</param>
    /// <param name="amp">amplitude êUìÆÇÃëÂÇ´Ç≥ÇæÇ∆évÇÌÇÍÇÈ</param>
